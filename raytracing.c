...
void* raytracing(void *id)
{
    struct input* temp = (struct input*)id;
    printf("We are now running on thread %d\n", temp->id);
    printf("We are now running on %d threads\n", temp->total);
    uint8_t *pixels = temp->pixel;
    color background_color;
    background_color[0] = temp->background_color[0];
    background_color[1] = temp->background_color[1];
    background_color[2] = temp->background_color[2];
    rectangular_node rectangulars = temp->rectangulars;
    sphere_node spheres = temp->spheres;
    light_node lights = temp->lights;
    const viewpoint *view = temp->view;
    int width = temp->width;
    int height = temp->height;

    int total_thread = temp->total;
    //calculate hpw much height to calculate in each thread
    int h = height / total_thread;
    ...
    for(int q = 0; q < total_thread; q++){
      if(q == temp->id){
        int factor = sqrt(SAMPLES);
        for (int j = h*q ; j < h*(q+1); j++) {
